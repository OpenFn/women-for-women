...
fn(state => {
  ...
  const diffdate = '17022023';

  const files = state.data
    .filter(file => {
      // console.log(file.name.split('.')[0].toLowerCase());
      // console.log(file.name.split('.')[0].toLowerCase().includes(fileNames));
      return file.name.split('.')[0].toLowerCase().includes(fileNames) && file.name.split('.')[1] === 'csv';
    })
    .map(file => {
      const dateTime = file.name.split('.')[0].match(/\d+$/);
  ...
  // .flat()
  // .sort((a, b) => {
  //   const distancea = Math.abs(diffdate - a);
  //   const distanceb = Math.abs(diffdate - b);
...
// fn(state => {
//   const fileNames = ['wfwi donors 17022023'];
//   console.log('Files to sync: ', fileNames);
